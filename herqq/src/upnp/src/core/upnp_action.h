...
#ifndef UPNP_ACTION_H_
#define UPNP_ACTION_H_
 ...
 *
 * \headerfile upnp_action.h HActionInvokeCallback
 *
 * \ingroup devicemodel
 */
typedef Functor<bool, H_TYPELIST_3(const QUuid&, qint32, const HActionOutputArguments&)>
 ...
 *
 * You can invoke an \c %HAction synchronously and asynchronously. To perform a
 * synchronous invocation, you can call invoke() or you can call beginInvoke()
 * immediately followed by waitForInvoke(). To perform an asynchronous invocation,
 * you have to call beginInvoke(). It is important to note that although \c %HAction
 * is derived from \c QObject, neither the invoke() nor beginInvoke() has thread affinity.
 * From this follows that you can perform invocation from any thread.
 *
 * \headerfile upnp_action.h HAction
 *
 * \ingroup devicemodel
 *
 * \sa HService
 ...
 * \li calling invoke() from the thread in which the \c %HAction resides is not advised,
 * since this is the thread in which the other objects of the \ref devicemodel
 * live as well. The invoke() is a blocking call and it could delay the
 * event processing of all the objects in the thread.
 */
...
public:
     ...
     * \endcode
     *
     * \return a copy of the input arguments the action expects.
     *
     ...
     *
     * Most often action invocation is preceded by calling this and
     * passing this to the invoke() without modification. For example,
     *
     *
     * \return a copy of the output arguments of the action.
     *
     ...
     *
     * \return the name of the output argument that is marked as the action's return value,
     * or an empty string, if no output argument has been marked as
     ...
     * \li the invocation may be slightly slower compared to the invocation speed
     * of the synchronous counterpart invoke().
     *
     * When the action has successfully completed, the signal invokeComplete()
     * is emitted. If the action invocation failed for whatever reason, the
     * signal invokeFailed() is emitted. You can also call
     * waitForInvoke() to wait until the invocation is complete.
     *
     * \param inArgs specifies the input arguments for the action invocation
     *
     * \return the ID used to identify the asynchronous operation.
     *
     * \remark
     * \li The appropriate signal is always emitted,
     * even if you have called waitForInvoke().
     * \li waitForInvoke() will complete after the appropriate signal
     * has been sent.
     *
     ...
     * You can also call waitForInvoke() to wait until the invocation is complete.
     *
     ...
     * This can be used to ensure that only the caller will get the result of the action.
     ...
     * waitForInvoke()
     * \li the completion callback is always called first and the signal is fired next if you
     * requested it (via the callback return value)
     * \li waitForInvoke() will complete after the callback is called and the possibly
     * requested signal has been sent.
     *
     * \sa waitForInvoke(), invoke(), inputArguments()
     */
    QUuid beginInvoke(
     ...
     * by beginInvoke().
     *
     * You can use this method to block the current thread until the specified
     * action invocation is complete.
     *
     * \param invokeId specifies the action invocation previously started by
     * beginInvoke(). If the invokeId is invalid, the method returns immediately with
     * an error code.
     *
    ...
    InvocationWaitReturnValue waitForInvoke(
        QUuid invokeId, HActionOutputArguments* outArgs = 0, qint32 timeout = -1);
     ...
     * occurred.
     *
     * \remark You should not call this method from the thread in which the
     * \c %HAction lives, since this is the thread in which the other objects
     * of the \ref devicemodel live as well. Because this is a blocking call,
     * it could delay the event processing of all the objects in the thread.
     *
...
Q_SIGNALS:
    ...
    /*!
     * This signal is emitted when an asynchronous action invocation has been completed.
     *
     * \param invokeId specifies the ID of the invocation that completed.
     * \param outArgs specifies the output arguments of the completed invocation.
     */
    void invokeComplete(
        const QUuid& invokeId, const Herqq::Upnp::HActionOutputArguments& outArgs);

    /*!
     * This signal is emitted when an asynchronous action invocation failed.
     *
     * \param invokeId specifies the ID of the invocation that failed.
     * \param errCode specifies the error code the action invocation returned.
     */
    void invokeFailed(const QUuid& invokeId, qint32 errCode);
