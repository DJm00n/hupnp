 ...
 ******************************************************************************/
ServiceDescriptionFetcherImpl::~ServiceDescriptionFetcherImpl()
{
}
...
HObjectCreationParameters::HObjectCreationParameters() :
    ...
    m_actionInvokeCreator(0), m_createDefaultObjects(false),
    ...
    HService* service, const QDomElement& serviceDefinition)
{
    ...
    service->h_ptr->m_serviceId =
        XmlUtils::readElementValue("serviceId"  , serviceDefinition);

    service->h_ptr->m_serviceType =
        XmlUtils::readElementValue("serviceType", serviceDefinition);

    QString tmp = XmlUtils::readElementValue("SCPDURL", serviceDefinition);

    if (!tmp.startsWith('/')) { tmp.insert(0, '/'); }
    service->h_ptr->m_scpdUrl = tmp;

    tmp = XmlUtils::readElementValue("controlURL" , serviceDefinition);
    if (!tmp.startsWith('/')) { tmp.insert(0, '/'); }
    service->h_ptr->m_controlUrl = tmp;

    tmp = XmlUtils::readElementValue("eventSubURL", serviceDefinition);
    if (!tmp.startsWith('/')) { tmp.insert(0, '/'); }
...
void HObjectCreator::parseServiceDescription(HService* service)
{
    ...
    {
        throw HParseException(
            QObject::tr("Service [%1] is missing mandatory service state table.").arg(
    ...
    const QDomElement& stateVariableElement)
{
    ...
    {
        throw HParseException(QObject::tr("Invalid value for [sendEvents]."));
    }
    ...
    {
        throw HParseException(QObject::tr("Invalid value for [multicast]."));
    }
    ...
    QString name =
         XmlUtils::readElementValue("name", stateVariableElement);

    QString dataType =
         XmlUtils::readElementValue("dataType", stateVariableElement);

    bool wasDefined = false;
    QString defaultValue =
         XmlUtils::readElementValue("defaultValue", stateVariableElement, &wasDefined);
    ...
    try
    {
        if (dataType.compare(HUpnpDataTypes::string_str()) == 0)
        {
            ...
            QDomNodeList allowedValueListNodeList =
                stateVariableElement.elementsByTagName("allowedValueList");

            if (allowedValueListNodeList.size())
            {
                QDomNodeList allowedValueElements =
                    allowedValueListNodeList.at(0).toElement().elementsByTagName("allowedValue");

                for (int i = 0; i < allowedValueElements.size(); ++i)
                {
                    allowedValues.push_back(
                        allowedValueElements.at(i).toElement().text());
                }
            }
        ...
        HUpnpDataTypes::DataType dataTypeEnumValue = HUpnpDataTypes::dataType(dataType);

        if (HUpnpDataTypes::isNumeric(dataTypeEnumValue))
        {
            QDomNodeList allowedValueRangeNodeList =
                stateVariableElement.elementsByTagName("allowedValueRange");
            if (allowedValueRangeNodeList.size())
            {
                QDomElement allowedValueRangeElement =
                    allowedValueRangeNodeList.at(0).toElement();

                QString minimumStr =
                    XmlUtils::readElementValue("minimum", allowedValueRangeElement);

               QString maximumStr =
                    XmlUtils::readElementValue("maximum", allowedValueRangeElement);

               QString stepStr =
                   XmlUtils::readElementValue("step", allowedValueRangeElement);
    ...
    const HService::HActionMapT& definedActions)
{
    ...
    QString name = XmlUtils::readElementValue("name", actionElement);
    ...
    if (!argumentListElement.isNull())
    {
        ...
        {
            QString name   =
                XmlUtils::readElementValue("name", argumentElement);

            QString dirStr =
                XmlUtils::readElementValue("direction", argumentElement);

            bool retValWasDefined = false;
            XmlUtils::readElementValue("retval", argumentElement, &retValWasDefined);

            QString relatedStateVar =
                XmlUtils::readElementValue("relatedStateVariable", argumentElement);
            ...
            if (dirStr.compare("out", Qt::CaseInsensitive) == 0)
            {
                if (retValWasDefined)
                {
                    ...
                    {
                        throw HParseException(
                            QObject::tr("[retval] must be the first [out] argument."));
                    }
            ...
            else if (dirStr.compare("in", Qt::CaseInsensitive) == 0)
            {
                if (firstOutArgFound)
                {
                    throw HParseException(QObject::tr(
                        "Invalid argument order. Input arguments must all come before output arguments."));
                }
    ...
    const QDomElement& iconListElement)
{
    ...
    {
        QUrl iconUrl = XmlUtils::readElementValue("url", iconElement);
...
HDeviceInfo* HObjectCreator::parseDeviceInfo(const QDomElement& deviceElement)
{
    ...
    QString deviceType       =
        XmlUtils::readElementValue("deviceType"      , deviceElement);

    QString friendlyName     =
        XmlUtils::readElementValue("friendlyName"    , deviceElement);

    QString manufacturer     =
        XmlUtils::readElementValue("manufacturer"    , deviceElement);

    QString manufacturerURL  =
        XmlUtils::readElementValue("manufacturerURL" , deviceElement);

    QString modelDescription =
        XmlUtils::readElementValue("modelDescription", deviceElement);

    QString modelName        =
        XmlUtils::readElementValue("modelName"       , deviceElement);

    QString modelNumber      =
        XmlUtils::readElementValue("modelNumber"     , deviceElement);

    QUrl    modelUrl         =
        XmlUtils::readElementValue("modelURL"        , deviceElement);

    QString serialNumber     =
        XmlUtils::readElementValue("serialNumber"    , deviceElement);

    HUdn udn(XmlUtils::readElementValue("UDN"        , deviceElement));

    QString upc              =
        XmlUtils::readElementValue("UPC"             , deviceElement);
    ...
    QString tmp =
        XmlUtils::readElementValue("presentationURL", deviceElement, &wasDefined);
    ...
    const QDomElement& serviceListElement, HDevice* device)
{
    ...
    {
        QString serviceId   =
            XmlUtils::readElementValue("serviceId", serviceElement);

        QString serviceType =
            XmlUtils::readElementValue("serviceType", serviceElement);

        if (serviceId.isEmpty())
        {
            throw InvalidServiceDescription(
                QObject::tr("Service ID is not defined."));
        }
        if (serviceType.isEmpty())
        {
            throw InvalidServiceDescription(
                QObject::tr("Service Type is not defined."));
        }

        HService* service = services.value(HResourceType(serviceType));
        ...
        if (!service)
        {
            ...
            else
            {
                QString err(QObject::tr(
                    "Could not create a service of type [%1] with ID %2").arg(
                        serviceType, serviceId));

                throw HOperationFailedException(err);
            }
        }
        ...
        if (!service->serviceType().isValid() ||
            !service->serviceId().isValid())
        {
            qDeleteAll(services);
            throw HOperationFailedException(
                QObject::tr("Service: [%1] was not properly initialized").arg(
                    serviceType));
        }
...
HDeviceController* HObjectCreator::parseDevice(const QDomElement& deviceElement)
{
    ...
    if (!device)
    {
        ...
        {
            throw HOperationFailedException(
                QObject::tr("Failed to create a UPnP device matching the type [%1].").arg(
...
HDeviceController* HObjectCreator::createRootDevice()
{
    ...
    createdDevice->m_device->h_ptr->m_locations =
        m_creationParameters.m_appendUdnToDeviceLocation ?
            generateLocations(createdDevice->m_device->deviceInfo().udn(), m_creationParameters.m_deviceLocations) :
