...
void verifySpecVersion(const QDomElement& rootElement)
{
    ...
    if (specVersionElement.isNull())
    {
        throw HIllegalArgumentException(QObject::tr(
            "Invalid device description: missing specVersion element"));
    }

    QString minorVersion = XmlUtils::readElementValue("minor", specVersionElement);
    QString majorVersion = XmlUtils::readElementValue("major", specVersionElement);
...
qint32 readConfigId(const QDomElement& rootElement)
{
    ...
    QString cid    = XmlUtils::readElementValue("configId", rootElement);
...
QString urlsAsStr(const QList<QUrl>& urls)
{
    ...
    for(qint32 i = 0; i < urls.size(); ++i)
    {
        retVal.append(QString("#%1 %2\n").arg(
            QString::number(i), extractBaseUrl(urls[i]).toString()));
    }
