        ...
        m_data    ()
{
    ...
    if (callback.isEmpty() || !callback.isValid() ||
    ...
    const QString& seq, const QString& contents)
{
    ...
    if (tmp.m_callback.isEmpty() || !tmp.m_callback.isValid())
    {
        return BadRequest;
    }
...
HEndpoint NotifyRequest::host() const
{
    return HEndpoint(QHostAddress(m_callback.host()), m_callback.port());
}

QUrl NotifyRequest::deliveryPath() const
{
