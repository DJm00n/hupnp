...
qint32 parseCacheControl(const QString& str)
{
    ...
    {
        throw HParseException(QObject::tr("Invalid Cache-Control field value: %1").arg(str));
    }
...
void HSsdpPrivate::processNotify(const QString& msg, const HEndpoint& /*from*/)
{
    ...
    {
        HLOG_WARN(QObject::tr("Ignoring an invalid SSDP presence announcement: [%1].").arg(nts));
    }
}
